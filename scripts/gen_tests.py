cases = [
    ...
    ("103", "E4M3 Subnormal (0x04) * 1.0", 0, 0, 127, 127, 0x04, 0x38, 64),
    ("104", "E4M3 Negative Subnormal (0x84) * 1.0", 0, 0, 127, 127, 0x84, 0x38, -64),
    ...
    ("108", "E5M2 Subnormal (0x20) * 1.0", 1, 1, 127, 127, 0x20, 0x3C, 64),
    ("109", "E5M2 Negative Subnormal (0xA0) * 1.0", 1, 1, 127, 127, 0xA0, 0x3C, -64),
    ("110", "E5M2 Infinity * 1.0", 1, 1, 127, 127, 0x7C, 0x3C, 2139095040),
    ("111", "E5M2 Negative Infinity * 1.0", 1, 1, 127, 127, 0xFC, 0x3C, -8388608),
    ("112", "E5M2 Zero * Infinity = NaN", 1, 1, 127, 127, 0x00, 0x7C, 2143289344),
...
with open("test/TEST_MIN_MAX_ZERO.yaml", "w") as f:
    for tc, comment, fa, fb, sa, sb, a_val, b_val, exp in cases:
        ...
        a_list = ", ".join([hex(a_val)] * 32)
        b_list = ", ".join([hex(b_val)] * 32)

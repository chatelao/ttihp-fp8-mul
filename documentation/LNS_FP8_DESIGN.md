...
In a standard bit-serial implementation (e.g., `OCP_MX_SERIAL.md`), multiplication is usually the most complex operation, requiring a shift-and-add structure that takes $O(N^2)$ or $O(N \times K)$ cycles.

By applying LNS, the bit-serial multiplication ($M_A \times M_B$) is transformed into a **bit-serial addition** of the logarithms.
*   **Datapath**: A single 1-bit full adder and one carry flip-flop.
*   **Control**: A bit-counter to track the exponent and mantissa fields as they stream in.
*   **Logic**: The product bit-stream is generated by adding the bits of the two log-operands (exponent and mantissa concatenated) with carry propagation.

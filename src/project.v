...
module tt_um_chatelao_fp8_multiplier #(
    ...
    reg nan_sticky;
    reg inf_sticky;
    wire block_nan = (ENABLE_SHARED_SCALING && (scale_a_val == 8'hFF || scale_b_val == 8'hFF));
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            inf_sticky <= inf_sticky || mul_inf_lane0_val || mul_inf_lane1_val;
        end
    end

    wire final_nan = nan_sticky || block_nan;
    wire final_inf = inf_sticky && !final_nan;

    wire [31:0] final_scaled_result = ENABLE_SHARED_SCALING ? aligned_lane0_res : acc_out_ext;

    wire [31:0] special_result_override = final_nan ? 32'h7FC00000 : // IEEE-754 Quiet NaN
                                          final_inf ? (acc_out[ACCUMULATOR_WIDTH-1] ? 32'hFF800000 : 32'h7F800000) : // IEEE-754 Inf
                                          final_scaled_result;
    ...
    accumulator #(
        ...
        .load_data(special_result_override),

...
module tt_um_chatelao_fp8_multiplier #(
        ...
        if (SUPPORT_MX_PLUS) begin : gen_mx_plus
            ...
            always @(posedge clk) begin
                if (!rst_n) begin
                    ...
                    if (cycle_count == 12'd0 && !ui_in[7])
                        bm_index_a <= uio_in[4:0];
                    if (cycle_count == 12'd2)
    ...
    generate
        ...
        if (ENABLE_SHARED_SCALING) begin : gen_scale_b
            ...
            always @(posedge clk) begin
                ...
                else if (ena && cycle_count == 12'd2) scale_b <= ui_in;
            end
        ...
        if (SUPPORT_MIXED_PRECISION) begin : gen_format_b
            ...
            always @(posedge clk) begin
                ...
                else if (ena && cycle_count == 12'd2) format_b <= uio_in[2:0];
            end
    ...
    wire [11:0] last_cycle        = SUPPORT_SERIAL ? (capture_cycle + 4 * SERIAL_K_FACTOR) :
                                    (actual_packed_mode ? 12'd24 : 12'd40);
    ...
    wire acc_en    = SUPPORT_PIPELINING ?
                     ...
                     ((cycle_count >= 12'd3 && cycle_count <= last_stream_cycle) && (state == STATE_STREAM) && serial_sample_tick);
    wire acc_clear = (cycle_count <= 12'd2) && (state != STATE_STREAM);
    ...
    wire [11:0] output_offset = cycle_count - capture_cycle;
    wire output_shift_tick = SUPPORT_SERIAL ? ((output_offset % SERIAL_K_FACTOR) == 12'd0) : 1'b1;
    ...
    accumulator #(
        ...
        .shift_en(ena && state == STATE_OUTPUT && cycle_count > capture_cycle && cycle_count < last_cycle && output_shift_tick),
    ...
    always @(posedge clk) begin
        if (f_past_valid && $past(rst_n) && rst_n) begin
            ...
            if (SUPPORT_MX_PLUS) begin
                ...
                if ($past(cycle_count) != 12'd2) begin
    ...
    always @(*) begin
        if (rst_n) begin
            if (state != STATE_OUTPUT) begin
                ...
                if (SUPPORT_SERIAL) begin
                    // In serial mode, output is held for SERIAL_K_FACTOR cycles
                    case ((cycle_count - capture_cycle - 1) / SERIAL_K_FACTOR)

...
module fp8_aligner_serial #(
    ...
    reg [MAX_DELAY-1:0] strobe_delay_line;
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            delay_line <= {MAX_DELAY{1'b0}};
            strobe_delay_line <= {MAX_DELAY{1'b0}};
        end else if (ena) begin
            delay_line <= {delay_line[MAX_DELAY-2:0], prod_bit};
            strobe_delay_line <= {strobe_delay_line[MAX_DELAY-2:0], strobe};
        end
    end
    ...
    wire [MAX_DELAY:0] full_strobe_chain = {strobe_delay_line, strobe};
    ...
    // Only output bits when active.
    assign aligned_bit = active ? res_bit : 1'b0;

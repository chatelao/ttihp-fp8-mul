...
module fp8_aligner_serial #(
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) delay_line <= {MAX_DELAY{1'b0}};
            ...
            if (strobe) delay_line <= {MAX_DELAY{1'b0}}; // Reset delay line for new element
            else delay_line <= {delay_line[MAX_DELAY-2:0], prod_bit};
        end
    end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) carry_neg <= 1'b0;
            ...
            if (strobe) carry_neg <= 1'b0; // Reset carry for new element

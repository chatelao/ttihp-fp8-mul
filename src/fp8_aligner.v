...
module fp8_aligner (
    ...
    wire signed [10:0] shift_amt_full;
    assign shift_amt_full = $signed(exp_sum) - 11'sd5;
    ...
    always @(*) begin : align_logic
        // Initialize all to avoid latches and comply with Verilog-2005
        s_val = {8'd0, prod};
        r_val = 40'd0;
        p_val = 40'd0;
        n_val = 6'd0;
        ...
        p_val[0] = s_val[0];
        for (idx = 1; idx < 40; idx = idx + 1) begin
            p_val[idx] = p_val[idx-1] | s_val[idx];
        end
        ...
        if (shift_amt_full >= 0) begin
            ...
            if (shift_amt_full <= -11'sd40) begin
                ...
                n_val = -shift_amt_full[5:0];
                r_val = s_val >> n_val;
                rb_val = (n_val > 0) ? (s_val >> (n_val - 1)) & 1'b1 : 1'b0;
                sticky_val = (n_val > 1) ? (p_val >> (n_val - 2)) & 1'b1 : 1'b0;
            end
        ...
        if (sign) begin
            // Negative: Saturation at -2^31 (0x80000000).

...
module fp8_mul_serial #(
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            ea_reg <= {INTERNAL_EXP_WIDTH{1'b0}}; eb_reg <= {INTERNAL_EXP_WIDTH{1'b0}};
            bias_a_reg <= {INTERNAL_BIAS_WIDTH{1'b0}}; bias_b_reg <= {INTERNAL_BIAS_WIDTH{1'b0}};
    ...
    wire signed [EXP_SUM_WIDTH:0] b_sum_wide = $signed(bias_a_reg) + $signed(bias_b_reg);
    wire signed [EXP_SUM_WIDTH-1:0] b_sum = b_sum_wide[EXP_SUM_WIDTH-1:0];
    assign exp_sum = ea_s + eb_s - (b_sum - 7'sd7);
    /* verilator lint_on WIDTHEXPAND */
    /* verilator lint_on WIDTHTRUNC */

...
async def test_special_value_detection(dut):
    ...
    # 1. E4M3 NaN Detection (0x7F)
    await reset_dut(dut)
    # Cycle 1: Load Scale A and Format=E4M3
    dut.ui_in.value = 127
    dut.uio_in.value = 0 # E4M3
    await ClockCycles(dut.clk, k_factor)
    # Cycle 2: Load Scale B
    await ClockCycles(dut.clk, k_factor)
    # Cycle 3: Stream first element (NaN)
    dut.ui_in.value = 0x7F # NaN
    dut.uio_in.value = 0x38 # 1.0
    await ClockCycles(dut.clk, k_factor)
    await Timer(1, unit="ns")
    ...
    if support_e5m2:
        # 2. E5M2 Infinity Detection (0x7C)
        await reset_dut(dut)
        # Cycle 1: Load Scale A and Format=E5M2
        dut.ui_in.value = 127
        dut.uio_in.value = 1 # E5M2
        await ClockCycles(dut.clk, k_factor)
        # Cycle 2: Load Scale B
        await ClockCycles(dut.clk, k_factor)
        # Cycle 3: Stream first element (Inf)
        dut.ui_in.value = 0x7C # Inf
        dut.uio_in.value = 0x3C # 1.0
        await ClockCycles(dut.clk, k_factor)
        await Timer(1, unit="ns")
        assert dut.mul_nan_lane0.value == 0
        assert dut.mul_inf_lane0.value == 1

        # 3. E5M2 NaN Detection (0x7D)
        await reset_dut(dut)
        # Cycle 1: Load Scale A and Format=E5M2
        dut.ui_in.value = 127
        dut.uio_in.value = 1 # E5M2

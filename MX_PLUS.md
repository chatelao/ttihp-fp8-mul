...
- **Tasks**:
  ...
  - Option A: Add Cycle 41 to load `BM_METADATA_A` and `BM_METADATA_B`.
  - Option B: Repurpose bits in existing `LOAD_SCALE` cycles (Cycles 1-2).
...
- **Tasks**:
  - Add registers `bm_index_a`, `bm_index_b`, and `mx_metadata_a/b` to the top-level FSM in `src/project.v`.
  - Ensure these registers are reset correctly and held stable during the `STREAM` phase.
...
- **Goal**: Implement the MX+ numerical semantics.
- **Tasks**:
  - Modify the decoders for MXFP8, MXFP6, and MXFP4.
  - If `is_bm` is active:
  ...
  - If `is_bm` is inactive:
    - Use standard OCP MX decoding.
...
- **Tasks**:
  - Update the internal multiplier in `fp8_mul.v` to handle the extended mantissa bits.
...
- **Tasks**:
  - Run Yosys synthesis to measure the gate count delta of the MX+ extension.
